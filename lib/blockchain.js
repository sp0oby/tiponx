...
export const sendEthereumTip = async (recipientAddress, amount, currency = 'ETH') => {
  try {
...
export const sendSolanaTip = async (recipientAddress, amount, currency = 'SOL') => {
  try {
...
export const sendTip = async (chain, token, recipientAddress, amount) => {
  try {
    if (chain === 'Ethereum') {
...
export async function signEthereumUpvote(creatorHandle) {
...
export async function signSolanaUpvote(creatorHandle) {
  const message = `I want to upvote creator ${creatorHandle} on TipOnX`
...
export async function verifyEthereumSignature(message, signature, address) {
  try {
