...
// Store wallet state in a more persistent way for development mode
const getGlobalWalletState = () => {
  if (typeof window !== 'undefined') {
    window._walletState = window._walletState || {
      ethProvider: null,
      ethSigner: null,
      solPublicKey: null,
      solConnection: null,
...
// Use getters and setters for cached values to handle development mode
const getCachedEthProvider = () => getGlobalWalletState().ethProvider;
const getCachedEthSigner = () => getGlobalWalletState().ethSigner;
const setCachedEthProvider = (provider) => { getGlobalWalletState().ethProvider = provider; };
const setCachedEthSigner = (signer) => { getGlobalWalletState().ethSigner = signer; };
...
const sendEthereumTip = async (recipientAddress, amount, currency = 'ETH') => {
  try {
    ...
    if (getCachedEthProvider() && getCachedEthSigner()) {
      provider = getCachedEthProvider();
      signer = getCachedEthSigner();
      ...
      setCachedEthProvider(provider);
      setCachedEthSigner(signer);
    }
...
// Use getters and setters for Solana cached values
const getCachedSolanaPublicKey = () => getGlobalWalletState().solPublicKey;
const getCachedSolanaConnection = () => getGlobalWalletState().solConnection;
const setCachedSolanaPublicKey = (publicKey) => { getGlobalWalletState().solPublicKey = publicKey; };
const setCachedSolanaConnection = (connection) => { getGlobalWalletState().solConnection = connection; };
...
const sendSolanaTip = async (recipientAddress, amount, currency = 'SOL') => {
  try {
    ...
    if (getCachedSolanaPublicKey() && getCachedSolanaConnection()) {
      publicKey = getCachedSolanaPublicKey();
      connection = getCachedSolanaConnection();
      ...
      // Cache for future use
      setCachedSolanaPublicKey(publicKey);
      setCachedSolanaConnection(connection);
    }
...
if (typeof window !== 'undefined') {
  // MetaMask events
        ...
        window.ethereum.on('accountsChanged', (accounts) => {
          if (accounts.length === 0) {
            ...
            setCachedEthProvider(null);
            setCachedEthSigner(null);
            localStorage.removeItem('ethWallet');
        // Dispatch custom event for components to update
        window.dispatchEvent(new CustomEvent('walletDisconnected', { detail: { chain: 'ETH' } }));
      }
    });
  ...
  if (window.solana) {
          ...
          setCachedSolanaPublicKey(null);
          setCachedSolanaConnection(null);
          localStorage.removeItem('solWallet');
      // Dispatch custom event for components to update
      window.dispatchEvent(new CustomEvent('walletDisconnected', { detail: { chain: 'SOL' } }));
    });
  }
}

// Function to restore wallet connections
const restoreWalletConnections = async () => {
  try {
    // Restore Ethereum connection if address exists
    const savedEthWallet = localStorage.getItem('ethWallet');
    if (savedEthWallet && window.ethereum) {
      try {
        // Request accounts silently first
        const accounts = await window.ethereum.request({ 
          method: 'eth_accounts'  // This doesn't prompt, just gets already-connected accounts
        });
        
        if (accounts.includes(savedEthWallet)) {
          // Wallet is still authorized, set up provider and signer
          const provider = new ethers.providers.Web3Provider(window.ethereum);
          const signer = provider.getSigner();
          setCachedEthProvider(provider);
          setCachedEthSigner(signer);
          console.log('Successfully restored Ethereum wallet connection');
        } else {
          // Try to reconnect with prompt
          const requestedAccounts = await window.ethereum.request({ 
            method: 'eth_requestAccounts'  // This will prompt if needed
          });
          
          if (requestedAccounts.includes(savedEthWallet)) {
            const provider = new ethers.providers.Web3Provider(window.ethereum);
            const signer = provider.getSigner();
            cachedEthProvider = provider;
            cachedEthSigner = signer;
            console.log('Successfully reconnected Ethereum wallet');
          } else {
            // Connected account doesn't match saved address
            console.log('Connected Ethereum account does not match saved address');
            localStorage.removeItem('ethWallet');
          }
        }
      } catch (error) {
        console.error('Failed to restore Ethereum connection:', error);
        localStorage.removeItem('ethWallet');
      }
    }

    // Restore Solana connection if address exists
    const savedSolWallet = localStorage.getItem('solWallet');
    if (savedSolWallet && window.solana?.isPhantom) {
      try {
        // Try to reconnect silently first
        const autoConnectResp = await window.solana.connect({ onlyIfTrusted: true });
        
        if (autoConnectResp.publicKey.toString() === savedSolWallet) {
          setCachedSolanaPublicKey(savedSolWallet);
          setCachedSolanaConnection(getSolanaConnection());
          console.log('Successfully restored Solana wallet connection');
        } else {
          // Try explicit connect if silent connect got different address
          const resp = await window.solana.connect();
          if (resp.publicKey.toString() === savedSolWallet) {
            cachedSolanaPublicKey = savedSolWallet;
            cachedSolanaConnection = getSolanaConnection();
            console.log('Successfully reconnected Solana wallet');
          } else {
            // Connected wallet doesn't match saved address
            console.log('Connected Solana wallet does not match saved address');
            localStorage.removeItem('solWallet');
          }
        }
      } catch (error) {
        console.error('Failed to restore Solana connection:', error);
        localStorage.removeItem('solWallet');
      }
    }
  } catch (error) {
    console.error('Error in restoreWalletConnections:', error);
  }
};

// Call restore function when module loads (client-side only)
if (typeof window !== 'undefined') {
  // Wait for window.ethereum and window.solana to be available
  const initializeWallets = () => {
    if ((window.ethereum || window.solana) && !getGlobalWalletState().initialized) {
      getGlobalWalletState().initialized = true;
  ...
  // Also try when the window loads (some wallets might inject late)
  window.addEventListener('load', initializeWallets);
