...
export async function verifyTwitter(handle, tweetUrl, tweetData) {
  const db = await getDb();
  const updates = { 
  ...
  // If we have tweet data with user description, include it
  if (tweetData?.includes?.users?.[0]?.description) {
    updates.description = tweetData.includes.users[0].description;
  }
